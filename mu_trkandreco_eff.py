...
def match_pftrack(g, trk_pt, trk_eta, trk_phi, pdg, dr_max=0.05, relpt_max=0.3):
    gpt, geta, gphi = float(g["pt"]), float(g["eta"]), float(g["phi"])
    if gpt <= 0:
        return None

    best, best_key = None, (1e9, 1e9)
    n = min(len(trk_pt), len(trk_eta), len(trk_phi), len(pdg))
    for i in range(n):
        pid = int(pdg[i])
        # Skip electron and pion tracks
        if abs(pid) in (11, 211):
            continue
        ...
        if key < best_key:
            best_key = key
            best = {"pt": pt, "eta": eta, "phi": phi, "pdgId": pid, "dr": dr, "relpt": relpt}
    return best
...
def main():
    ...
    for iz, fpath in enumerate(files):
        ...
        for ev in t:
            ...
            trk_eta = getattr(ev, "PFCands_trkEta", None)
            trk_phi = getattr(ev, "PFCands_trkPhi", None)
            trk_pt  = getattr(ev, "PFCands_trkPt", None)
            pf_pdg  = getattr(ev, "PFCands_pdgId", None)
            if any(x is None for x in (trk_eta, trk_phi, trk_pt, pf_pdg)):
                continue

            # Grab reco muon branches
            ...
            mu_gbl  = getattr(ev, "Muon_isGlobal", None)
            mu_trk  = getattr(ev, "Muon_isTracker", None)
            
            # Create a map and apply reco muon object sels
            reco_by_gen = {}
            if not any(x is None for x in (mu_pt, mu_eta, mu_phi, mu_m, mu_med, mu_gidx, mu_gbl, mu_trk)):
                for i in range(nMu):
                    gidx = int(mu_gidx[i]) if i < len(mu_gidx) else -1
                    if gidx < 0:
                        continue
                    if not bool(mu_med[i]):
                        continue
                    if not (bool(mu_trk[i]) and bool(mu_gbl[i])):
                        continue
                    prev = reco_by_gen.get(gidx)
                    if (prev is None) or (float(mu_pt[i]) > prev["pt"]):
                        reco_by_gen[gidx] = {"pt": float(mu_pt[i]), "eta": float(mu_eta[i]), "phi": float(mu_phi[i]), "m": float(mu_m[i])}

            # Compute track efficiency
            passed1 = {}
            for g in pair:
                # Grab pT and eta bins of the. muon pair
                ...
                tot1[ie][ip] += 1
                m = match_pftrack(g, trk_pt, trk_eta, trk_phi, pf_pdg, dr_max=0.05, relpt_max=0.3)
                if not m:
                    continue
            ...
            for g in pair:
                gidx = int(g["idx"])
                # Skip muons that failed the track efficiency
            ...
            # If both muons in event pass, compute dimuon reco mass
    ...
    eff1_path = f"{outbase}_track_efficiency.csv"
    write_eff_csv(eff1_path, eta_edges, pt_bins, tot1, hit1)
    print(f"Wrote: {eff1_path}")

    res_path = f"{outbase}_momentum_resolution.csv"
    ...
    eff2_path = f"{outbase}_reconstruction_efficiency.csv"
    write_eff_csv(eff2_path, eta_edges, pt_bins, tot2, hit2)
    print(f"Wrote: {eff2_path}")

    mass_path = f"{outbase}_reco_mass.csv"
