#!/usr/bin/env python3
"""
Summarize momentum resolution into (mu, sigma) per (eta, pT) bin.

Input CSV format (from your producer):
  gen_eta, reco_eta, gen_pt, reco_pt, resolution
  (eta_lo, eta_hi no longer required)

Eta bins are fixed for all samples: (0, 0.9, 1.2, 2.1, 2.4)

Output CSV columns:
  eta_lo, eta_hi, pt_lo, pt_hi, n, mu, mu_err, sigma, sigma_err

Errors:
  mu_err    = sigma / sqrt(n)              (standard error of the mean)
  sigma_err = sigma / sqrt(2*(n-1))        (approx. std error of sample std dev)
"""
