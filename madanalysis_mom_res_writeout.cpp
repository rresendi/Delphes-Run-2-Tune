                                     ...
                                     std::string charge = "")
{
  ...
  for (auto &obj : objects)
  {
    ...
    std::cout << "Track pdgid = " << obj.pdgid() << std::endl;
                        ...
                        const std::map<std::string, std::string> &parameters)
{
  ...
  std::ofstream f("momentum_resolution.csv");
  f << "event,reco_pt,gen_pt,resolution\n";
...
bool user::Execute(SampleFormat &sample, const EventFormat &event)
{
  ...
  vector<RecTrackFormat> gen_muons = filter_muons(event.rec()->tracks(), MUON_PT_MIN, MUON_PT_MAX, MUON_ETA_MAX, PDGID);
  vector<RecTrackFormat> gen_posMuons = filter_muons(gen_muons, MUON_PT_MIN,MUON_PT_MAX, MUON_ETA_MAX, PDGID, "+");
  vector<RecTrackFormat> gen_negMuons = filter_muons(gen_muons, MUON_PT_MIN,MUON_PT_MAX, MUON_ETA_MAX, PDGID, "-");

  // Sort all collections before event selection
  sortLeptonCollections(gen_muons, gen_posMuons, gen_negMuons);
  ...
  bool twoOSleptons = (gen_posMuons.size() > 0 && gen_negMuons.size() > 0);              // Require exactly two opposite-sign leptons (either muons or electrons)
  ...
  RecTrackFormat leading_pos_genMuon = gen_posMuons.at(0);
  RecTrackFormat leading_neg_genMuon = gen_negMuons.at(0);
  ParticleBaseFormat recoJPSI;
  recoJPSI += leading_pos_genMuon.momentum();
  recoJPSI += leading_neg_genMuon.momentum();
  ...
  if (gen_muons.size() > 0)
  {
    Manager()->FillHisto("LeadMuonPT", gen_muons.at(0).pt());
    Manager()->FillHisto("LeadMuonETA", gen_muons.at(0).eta());
    Manager()->FillHisto("LeadMuonPHI", gen_muons.at(0).phi());
  }
    if (gen_muons.size() > 1)
  {
    Manager()->FillHisto("SubleadMuonPT", gen_muons.at(1).pt());
    Manager()->FillHisto("SubleadMuonETA", gen_muons.at(1).eta());
    Manager()->FillHisto("SubleadMuonPHI", gen_muons.at(1).phi());
  }

  static unsigned long long evt = 0;
  auto& reco_muons = event.rec()->muons();

  for (const auto& mu : reco_muons)
  {
    const RecTrackFormat* best = nullptr;
    double bestDR = 1e9;

    for (const auto& gtrk : gen_muons)
    {
      double dR = calculateDeltaR(mu, gtrk);
      if (dR < bestDR) { bestDR = dR; best = &gtrk; }
    }

    if (!best) continue;
    if (bestDR > 0.05) continue;

    double gen_pt  = best->pt();
    double reco_pt = mu.pt();
    if (gen_pt <= 0) continue;

    double res = (reco_pt - gen_pt) / gen_pt;

    std::ofstream f("momentum_resolution.csv", std::ios::app);
    f << evt << "," << reco_pt << "," << gen_pt << "," << res << "\n";
  }
  ++evt;
